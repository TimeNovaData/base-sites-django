...
path = os.path.join(BASE_DIR, "static/public/img/")

are_images_converted = os.path.exists(path)
print(path)
...
def get_missing_images_from_new_directory():
    try:
        ...
        if os.path.exists(os.path.join(os.curdir, dir_name)):
            ...
            missing_images = [
                filename
                for filename in set(list_of_original_images)
...
def if_missing_images_exist():
    try:
        ...
        webp_images_in_curdir = [
            image for image in os.listdir(os.curdir) if image.endswith(".webp")
        ]

        converted_images_in_curdir = list(
            set([splitext(filename)[0] for filename in webp_images_in_curdir])
        )

        missing_images_in_curdir = [
            filename.split(".")[0]
            for filename in set(missing_images)
...
def if_images_converted():
    ...
    if os.path.exists(dir_name):
        ...
        if (
            len(get_images()) == len(if_all_images_exist)
            and (len(get_images()) > 0)
            and len(if_all_images_exist) > 0
        ):
        ...
        if if_images_converted() == "Images already exist":
            ...
            if type(get_missing_images_from_new_directory()) is list:
                if if_missing_images_exist() == True:
                    ...
                    elapsed = perf_counter() - start
                    print(
                        "\nSuccessfully restored the missing images in {0:.4f}s".format(
                            elapsed
                        )
                    )
                    input("\nPress any key to exit ")

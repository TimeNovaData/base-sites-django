...
django_vite_tags_DEV_MODE = getattr(
    settings, "django_vite_tags_DEV_MODE", False
)

# Default Vite server protocol (http or https)
django_vite_tags_DEV_SERVER_PROTOCOL = getattr(
    settings, "django_vite_tags_DEV_SERVER_PROTOCOL", "http"
)

# Default vite server hostname.
django_vite_tags_DEV_SERVER_HOST = getattr(
    settings, "django_vite_tags_DEV_SERVER_HOST", "localhost"
)

# Default Vite server port.
django_vite_tags_DEV_SERVER_PORT = getattr(
    settings, "django_vite_tags_DEV_SERVER_PORT", 3000
)

# Default Vite server path to HMR script.
django_vite_tags_WS_CLIENT_URL = getattr(
    settings, "django_vite_tags_WS_CLIENT_URL", "@vite/client"
)

django_vite_tags_REACT_REFRESH_URL = getattr(
    settings, "django_vite_tags_REACT_REFRESH_URL", "@react-refresh"
)
...
django_vite_tags_ASSETS_PATH = Path(
    getattr(settings, "django_vite_tags_ASSETS_PATH")
)

# Prefix for STATIC_URL
django_vite_tags_STATIC_URL_PREFIX = getattr(
    settings, "django_vite_tags_STATIC_URL_PREFIX", ""
)

django_vite_tags_STATIC_ROOT = (
    django_vite_tags_ASSETS_PATH
    if django_vite_tags_DEV_MODE
    else Path(settings.STATIC_ROOT) / django_vite_tags_STATIC_URL_PREFIX
)

# Path to your manifest file generated by Vite.
# Should by in "django_vite_tags_ASSETS_PATH".
django_vite_tags_MANIFEST_PATH = getattr(
    settings,
    "django_vite_tags_MANIFEST_PATH",
    django_vite_tags_STATIC_ROOT / "manifest.json",
)

# Motif in the 'manifest.json' to find the polyfills generated by Vite.
django_vite_tags_LEGACY_POLYFILLS_MOTIF = getattr(
    settings, "django_vite_tags_LEGACY_POLYFILLS_MOTIF", "legacy-polyfills"
)

django_vite_tags_STATIC_URL = urljoin(
    settings.STATIC_URL, django_vite_tags_STATIC_URL_PREFIX
)

# Make sure 'django_vite_tags_STATIC_URL' finish with a '/'
if django_vite_tags_STATIC_URL[-1] != "/":
    django_vite_tags_STATIC_URL += "/"
...
class DjangoViteAssetLoader:
        ...
        **kwargs: Dict[str, str],
        ...
        if not self._manifest or path not in self._manifest:
            raise RuntimeError(
                ...
                f"at {django_vite_tags_MANIFEST_PATH}"
            )
    ...
    def generate_vite_legacy_polyfills(
        ...
        **kwargs: Dict[str, str],
        ...
        for path, content in self._manifest.items():
            if django_vite_tags_LEGACY_POLYFILLS_MOTIF in path:
                return DjangoViteAssetLoader._generate_script_tag(
                    DjangoViteAssetLoader._generate_production_server_url(
                        content["file"]
                    ),
    ...
    def _parse_manifest(self) -> None:
        ...
        try:
            response = requests.get(django_vite_tags_MANIFEST_PATH)
            self._manifest = json.loads(response.text)
        except Exception as error:
            raise RuntimeError(
                f"Não foi possivel fazer o request do manifest"
                f"{django_vite_tags_MANIFEST_PATH} : {str(error)}"
            )
    ...
    def instance(cls):
        ...
        if cls._instance is None:
            ...
            if not django_vite_tags_DEV_MODE:
    ...
    def generate_vite_ws_client(cls, **kwargs: Dict[str, str]) -> str:
        ...
        return cls._generate_script_tag(
            cls._generate_vite_server_url(django_vite_tags_WS_CLIENT_URL),
            {"type": "module", **kwargs},
        )
    ...
    def _generate_vite_server_url(path: str) -> str:
        ...
        return urljoin(
            f"{django_vite_tags_DEV_SERVER_PROTOCOL}://"
            f"{django_vite_tags_DEV_SERVER_HOST}:{django_vite_tags_DEV_SERVER_PORT}",
            urljoin(django_vite_tags_STATIC_URL, path),
        )
    ...
    def generate_vite_react_refresh_url(cls) -> str:
        ...
        return f"""<script type="module">
            ...
            '{cls._generate_vite_server_url(django_vite_tags_REACT_REFRESH_URL)}'
    ...
    def _generate_production_server_url(path: str) -> str:
        ...
        if prefix := django_vite_tags_STATIC_URL_PREFIX:
            if not django_vite_tags_STATIC_URL_PREFIX.endswith("/"):
                prefix += "/"

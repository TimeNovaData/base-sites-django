...
ndt_vite_tags_DEV_MODE = getattr(settings, "ndt_vite_tags_DEV_MODE", False)

# Default Vite server protocol (http or https)
ndt_vite_tags_DEV_SERVER_PROTOCOL = getattr(
    settings, "ndt_vite_tags_DEV_SERVER_PROTOCOL", "http"
)

# Default vite server hostname.
ndt_vite_tags_DEV_SERVER_HOST = getattr(
    settings, "ndt_vite_tags_DEV_SERVER_HOST", "localhost"
)

# Default Vite server port.
ndt_vite_tags_DEV_SERVER_PORT = getattr(
    settings, "ndt_vite_tags_DEV_SERVER_PORT", 3000
)

# Default Vite server path to HMR script.
ndt_vite_tags_WS_CLIENT_URL = getattr(
    settings, "ndt_vite_tags_WS_CLIENT_URL", "@vite/client"
)

ndt_vite_tags_REACT_REFRESH_URL = getattr(
    settings, "ndt_vite_tags_REACT_REFRESH_URL", "@react-refresh"
)
...
ndt_vite_tags_ASSETS_PATH = Path(
    getattr(settings, "ndt_vite_tags_ASSETS_PATH")
)

# Prefix for STATIC_URL
ndt_vite_tags_STATIC_URL_PREFIX = getattr(
    settings, "ndt_vite_tags_STATIC_URL_PREFIX", ""
)

ndt_vite_tags_STATIC_ROOT = (
    ndt_vite_tags_ASSETS_PATH
    if ndt_vite_tags_DEV_MODE
    else Path(settings.STATIC_ROOT) / ndt_vite_tags_STATIC_URL_PREFIX
)

# Path to your manifest file generated by Vite.
# Should by in "ndt_vite_tags_ASSETS_PATH".
ndt_vite_tags_MANIFEST_PATH = getattr(
    settings,
    "ndt_vite_tags_MANIFEST_PATH",
    ndt_vite_tags_STATIC_ROOT / "manifest.json",
)

# Motif in the 'manifest.json' to find the polyfills generated by Vite.
ndt_vite_tags_LEGACY_POLYFILLS_MOTIF = getattr(
    settings, "ndt_vite_tags_LEGACY_POLYFILLS_MOTIF", "legacy-polyfills"
)

ndt_vite_tags_STATIC_URL = urljoin(
    settings.STATIC_URL, ndt_vite_tags_STATIC_URL_PREFIX
)

# Make sure 'ndt_vite_tags_STATIC_URL' finish with a '/'
if ndt_vite_tags_STATIC_URL[-1] != "/":
    ndt_vite_tags_STATIC_URL += "/"
...
class DjangoViteAssetLoader:
        ...
        **kwargs: Dict[str, str],
        ...
        if not self._manifest or path not in self._manifest:
            raise RuntimeError(
                ...
                f"at {ndt_vite_tags_MANIFEST_PATH}"
            )
    ...
    def generate_vite_legacy_polyfills(
        ...
        **kwargs: Dict[str, str],
        ...
        for path, content in self._manifest.items():
            if ndt_vite_tags_LEGACY_POLYFILLS_MOTIF in path:
                return DjangoViteAssetLoader._generate_script_tag(
                    DjangoViteAssetLoader._generate_production_server_url(
                        content["file"]
                    ),
    ...
    def generate_vite_ws_client(cls, **kwargs: Dict[str, str]) -> str:
        ...
        return cls._generate_script_tag(
            cls._generate_vite_server_url(ndt_vite_tags_WS_CLIENT_URL),
            {"type": "module", **kwargs},
        )
    ...
    def _generate_vite_server_url(path: str) -> str:
        ...
        return urljoin(
            f"{ndt_vite_tags_DEV_SERVER_PROTOCOL}://"
            f"{ndt_vite_tags_DEV_SERVER_HOST}:{ndt_vite_tags_DEV_SERVER_PORT}",
            urljoin(ndt_vite_tags_STATIC_URL, path),
        )
    ...
    def generate_vite_react_refresh_url(cls) -> str:
        ...
        return f"""<script type="module">
            ...
            '{cls._generate_vite_server_url(ndt_vite_tags_REACT_REFRESH_URL)}'
    ...
    def _generate_production_server_url(path: str) -> str:
        ...
        if prefix := ndt_vite_tags_STATIC_URL_PREFIX:
            if not ndt_vite_tags_STATIC_URL_PREFIX.endswith("/"):
                prefix += "/"
